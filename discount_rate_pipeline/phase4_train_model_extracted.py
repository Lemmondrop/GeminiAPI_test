"""
Phase 4 v2: train the discount-rate model with extracted segment features.

Input:
  - training_features_segmented.csv

Outputs:
  - model_v2.pkl
  - feature_importance_v2.csv
  - predictions_cv_v2.csv
  - model_metrics_v2.csv

This file intentionally coexists with phase4_train_model.py so the previous
...
INPUT_CSV = os.environ.get("TRAINING_CSV", "training_features_segmented.csv")
MODEL_OUT = os.environ.get("MODEL_OUT", "model_v2.pkl")
IMPORTANCE_OUT = os.environ.get("IMPORTANCE_OUT", "feature_importance_v2.csv")
PREDICTIONS_OUT = os.environ.get("PREDICTIONS_OUT", "predictions_cv_v2.csv")
METRICS_OUT = os.environ.get("METRICS_OUT", "model_metrics_v2.csv")
...
def engineer_features(train: pd.DataFrame) -> tuple[pd.DataFrame, list[str], dict]:
    ...
    segment_features.append("segment_unknown") if "segment_unknown" in train.columns else None
    ...
    features = BASE_FEATURES + segment_features + available_extra + MISSING_FLAG_FEATURES

    # Median imputation is intentionally kept simple because the dataset is small.
    medians = train[features].median(numeric_only=True)
    X = train[features].fillna(medians)

    return X, features, medians.to_dict()


def main() -> None:
    train = load_training_data(INPUT_CSV)
    X, features, medians = engineer_features(train)
